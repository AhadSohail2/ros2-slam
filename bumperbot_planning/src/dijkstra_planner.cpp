...
namespace bumperbot_planning
{
    ...
    void DijkstraPlanner::goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr goal)
    {
        if (!map_)
        {
        ...
        auto path = plan(map_to_base_pose, goal->pose);

        if (!path.poses.empty())
        {
            RCLCPP_INFO(get_logger(), "Shotest Path Found");
    ...
    {
        std::vector<std::pair<int, int>> explore_direction = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
        std::priority_queue<GraphNode, std::vector<GraphNode>, std::greater<GraphNode>> pending_nodes;
        std::vector<GraphNode> visited_node;
        ...
        while (!pending_nodes.empty() && rclcpp::ok())
        {
            ...
            GraphNode goalNode = worldToGrid(goal);

            if (goalNode == active_node)
            {
                break;
            }
            ...
            for (const auto &dir : explore_direction)
            {
                ...
                if (std::find(visited_node.begin(), visited_node.end(), new_node) == visited_node.end() && poseOnMap(new_node) && map_->data.at(poseToCell(new_node) == 0))
                {
                    new_node.cost = active_node.cost + 1;
                    new_node.prev = std::make_shared<GraphNode>(active_node);
                    pending_nodes.push(new_node);
                    visited_node.push_back(new_node);
                }
            }
